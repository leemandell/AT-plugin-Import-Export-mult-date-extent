...
package edu.byu.plugins.importExport.modsExport;
...
public class BYU_MODSExport {
    ...
    public List getModsElements(DigitalObjects digitalObject, List modsGroup) {
		...
		OriginInfoType originInfo;
		DateType dateType;
		handleArchDescriptionDates(digitalObject, modsGroup);
		//deal with dates from linked resources
...
//            modsGroup.add(originInfo);
	...
	private void handleArchDescriptionDates(ArchDescription digitalObject, List modsGroup) {
		OriginInfoType originInfo;
		DateType dateType;
		for (ArchDescriptionDates date: digitalObject.getArchDescriptionDates()) {
			originInfo = obj.createOriginInfoType();
			dateType = obj.createDateType();
			System.out.println("Date expression: " + date.getDateExpression());
			if (containsValue(date.getDateExpression())) {
				dateType.setValue(date.getDateExpression());
				JAXBElement dateCreated =
					obj.createOriginInfoTypeDateCreated(dateType);
				originInfo.getPlaceOrPublisherOrDateIssued().add(dateCreated);
			}

			if (date.getIsoDateBegin() != null && !(date.getIsoDateBegin().equals(0))) {
				dateType = obj.createDateType();
				dateType.setPoint("start");
				dateType.setValue(date.getIsoDateBegin() + "");
				JAXBElement dateCreated =
					obj.createOriginInfoTypeDateCreated(dateType);
				originInfo.getPlaceOrPublisherOrDateIssued().add(dateCreated);
			}

			if (date.getIsoDateEnd() != null && !(date.getIsoDateEnd().equals(0))) {
				dateType = obj.createDateType();
				dateType.setPoint("end");
				dateType.setValue(date.getIsoDateEnd() + "");
				JAXBElement dateCreated =
					obj.createOriginInfoTypeDateCreated(dateType);
				originInfo.getPlaceOrPublisherOrDateIssued().add(dateCreated);
			}
...
//            DateType dateType = obj.createDateType();
			...
			handleArchDescriptionDates(resource, relatedItem.getModsGroup());
