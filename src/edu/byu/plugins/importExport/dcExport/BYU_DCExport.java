...
package edu.byu.plugins.importExport.dcExport;
...
public class BYU_DCExport {
	...
	public String convertDBRecordToXML(DigitalObjects digitalObject,boolean internalOnly,boolean standalone){
		...
		for (ArchDescriptionDates date: digitalObject.getArchDescriptionDates()) {
			if(date.getDateExpression()!=null && date.getDateExpression().length()>0){
				etype = new ElementType();
				etype.setValue(StringHelper.tagRemover(date.getDateExpression()));
				type = obj.createDate(etype);
				elements.add(type);
			} else {
				etype = new ElementType();
				etype.setValue(date.getDateBegin()+"-"+date.getDateEnd());
				type = obj.createDate(etype);
				elements.add(type);
