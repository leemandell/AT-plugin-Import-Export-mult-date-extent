 ...
 */
public class BYU_AccessionImportXMLHandler extends ImportHandler {
     ...
     */
	public BYU_AccessionImportXMLHandler(ImportOptionsAccessions importOptions) {
		super();
	...
	public boolean importFile(File importFile, DomainImportController controller, InfiniteProgressPanel progressPanel) throws ImportException {
		...
		dateFields.add("dateExpression");
		dateFields.add("bulkDateBegin");
		dateFields.add("bulkDateEnd");
		dateFields.add("dateBegin");
		dateFields.add("dateEnd");

		HashSet<String> extentFields = new HashSet<String>();
		extentFields.add("extentNumber");
		extentFields.add("extentType");
		extentFields.add("containerSummary");
			...
			try {
				...
				for (RecordType record : accessionRecords.getRecord()) {
					...
					for (PropertyDescriptor descriptor: descriptors) {
						if (!dateFields.contains(descriptor.getName()) && !extentFields.contains(descriptor.getName()) ) {
					...
					addDateInfo(accession, record.getDateExpression(), record.getBulkDateBegin(), record.getBulkDateEnd(), record.getDateBegin(), record.getDateEnd());
					addExtentInfo(accession, record.getExtentNumber(), record.getExtentType(), record.getContainerSummary());
	...
	private void addDateInfo(Accessions accession, String dateExpression, BigInteger bulkDateBegin, BigInteger bulkDateEnd, BigInteger dateBegin, BigInteger dateEnd) throws UnknownLookupListException, InvocationTargetException, IllegalAccessException {
		if (dateExpression == null && dateBegin == null && dateEnd == null) {
		// do nothing
		} else {
			ArchDescriptionDates newDate = new ArchDescriptionDates(accession);
			ImportUtils.nullSafeSet(newDate, ArchDescriptionDates.PROPERTYNAME_DATE_EXPRESSION, dateExpression);
			if (bulkDateBegin != null) {
				ImportUtils.nullSafeSet(newDate, ArchDescriptionDates.PROPERTYNAME_ISOBULK_DATE_BEGIN, bulkDateBegin.toString());
			}
			if (bulkDateEnd != null) {
				ImportUtils.nullSafeSet(newDate, ArchDescriptionDates.PROPERTYNAME_ISOBULK_DATE_END, bulkDateEnd.toString());
			}
			if (dateBegin != null) {
				ImportUtils.nullSafeSet(newDate, ArchDescriptionDates.PROPERTYNAME_ISODATE_BEGIN, dateBegin.toString());
			}
			if (dateEnd != null) {
				ImportUtils.nullSafeSet(newDate, ArchDescriptionDates.PROPERTYNAME_ISODATE_END, dateEnd.toString());
			}
			accession.addArchdescriptionDate(newDate);
		}
	}

	private void addExtentInfo(Accessions accession, BigDecimal extentNumber, String extentType, String containerSummary) throws UnknownLookupListException, InvocationTargetException, IllegalAccessException {
		if (extentNumber == null || extentType == null) {
			//do nothing
		} else {
			ArchDescriptionPhysicalDescriptions physicalDescription = new ArchDescriptionPhysicalDescriptions(accession);
			ImportUtils.nullSafeSet(physicalDescription, ArchDescriptionPhysicalDescriptions.PROPERTYNAME_EXTENT_NUMBER, extentNumber);
			ImportUtils.nullSafeSet(physicalDescription, ArchDescriptionPhysicalDescriptions.PROPERTYNAME_EXTENT_TYPE, extentType);
			ImportUtils.nullSafeSet(physicalDescription, ArchDescriptionPhysicalDescriptions.PROPERTYNAME_CONTAINER_SUMMARY, containerSummary);
			accession.addPhysicalDesctiptions(physicalDescription);
		}
	}
	...
	private void addResource(Accessions accession,
		...
		if (resourceId != null) {
			...
			Resources resource;
			if (resourceCreation.equalsIgnoreCase(BYU_AccessionImportXMLHandler.RESOURCE_CREATION_NONE)) {
			...
			if (resourceDao.getNewRecordCreated() && resourceCreation.equalsIgnoreCase(BYU_AccessionImportXMLHandler.RESOURCE_CREATION_FULL))
			{
				try {
					...
					addDatesAndPhysicalDescriptions(accession, resource);
	...
	protected void addDatesAndPhysicalDescriptions(Accessions accession, Resources resource) {

		ArchDescriptionDates newDate;
		for (ArchDescriptionDates date: accession.getArchDescriptionDates()) {
			System.out.println("new date");
			newDate = new ArchDescriptionDates(resource);
			...
			resource.addArchdescriptionDate(newDate);
		}

		ArchDescriptionPhysicalDescriptions newPhysDesc;
		for (ArchDescriptionPhysicalDescriptions physDsesc: accession.getPhysicalDesctiptions()) {
			newPhysDesc = new ArchDescriptionPhysicalDescriptions(resource);
			newPhysDesc.setContainerSummary(physDsesc.getContainerSummary());
			newPhysDesc.setDimensions(physDsesc.getDimensions());
			newPhysDesc.setExtentNumber(physDsesc.getExtentNumber());
			newPhysDesc.setExtentType(physDsesc.getExtentType());
			newPhysDesc.setPhysicalDetail(physDsesc.getPhysicalDetail());
			resource.addPhysicalDesctiptions(newPhysDesc);
		}
	}
	...
	public static Vector<String> getResourceRecordCreationValues() {
		...
		returnVector.add(BYU_AccessionImportXMLHandler.RESOURCE_CREATION_NONE);
		returnVector.add(BYU_AccessionImportXMLHandler.RESOURCE_CREATION_STUB);
		returnVector.add(BYU_AccessionImportXMLHandler.RESOURCE_CREATION_FULL);
		return returnVector;
	}
}