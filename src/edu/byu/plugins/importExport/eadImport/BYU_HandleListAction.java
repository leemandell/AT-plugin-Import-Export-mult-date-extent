...
public class BYU_HandleListAction implements Action
{
    public void processElement(ArchDescription archDescription, Object o, InfiniteProgressPanel progressPanel)
    {
        ...
        if(type.equals("deflist")){
            ...
            listDefinition.setParentNote(BYU_EADInfo.getReferenceTOParentNotes());
            listDefinition.setSequenceNumber(BYU_EADInfo.getReferenceTOParentNotes().getChildren().size());
            BYU_EADInfo.getReferenceTOParentNotes().addRepeatingData(listDefinition);
            listDefinition.setPersistentId(BYU_EADInfo.getReferenceTOResources().getNextPersistentIdAndIncrement());
            BYU_EADInfo.addIdPairs(eadList.getId(),listDefinition.getPersistentId());
            if(title!=null)
            ...
            listOrdered.setParentNote(BYU_EADInfo.getReferenceTOParentNotes());
System.out.println("list seq = "+BYU_EADInfo.getReferenceTOParentNotes().getChildren().size());
            listOrdered.setSequenceNumber(BYU_EADInfo.getReferenceTOParentNotes().getChildren().size());
            BYU_EADInfo.getReferenceTOParentNotes().addRepeatingData(listOrdered);
            listOrdered.setPersistentId(BYU_EADInfo.getReferenceTOResources().getNextPersistentIdAndIncrement());
            BYU_EADInfo.addIdPairs(eadList.getId(),listOrdered.getPersistentId());
            if(title!=null)
