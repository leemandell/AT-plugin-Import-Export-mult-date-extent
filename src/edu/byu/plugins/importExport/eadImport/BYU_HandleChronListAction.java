...
public class BYU_HandleChronListAction implements Action
{
    public void processElement(ArchDescription archDescription, Object o, InfiniteProgressPanel progressPanel)
    {
        ...
        chronList.setParentNote(BYU_EADInfo.getReferenceTOParentNotes());
        int sequenceNum = BYU_EADInfo.getReferenceTOParentNotes().getChildren().size();
        chronList.setSequenceNumber(sequenceNum);
        chronList.setPersistentId(BYU_EADInfo.getReferenceTOResources().getNextPersistentIdAndIncrement());;
        BYU_EADInfo.addIdPairs(((Chronlist)o).getId(),chronList.getPersistentId());
        BYU_EADInfo.getReferenceTOParentNotes().addRepeatingData(chronList);
        Iterator i2;
