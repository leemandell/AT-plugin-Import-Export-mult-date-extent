...
package edu.byu.plugins.importExport.marcxmlExport;
...
public class BYU_MARCExport {
	...
	private void gatherDateInformation (Resources resource) {

		String certainty;
		int beginDate, endDate;

		for (ArchDescriptionDates archDescDate: resource.getArchDescriptionDates()) {

			if (archDescDate.getCertainty() != null && archDescDate.getCertainty()) {
				certainty = "circa ";
			} else {
				certainty = "";
			}
			if (archDescDate.getDateExpression() != null && archDescDate.getDateExpression().length() > 0) {
				dateInclusive = StringHelper.concat(", ", dateInclusive, archDescDate.getDateExpression());
			} else {
				dateInclusive = StringHelper.concat(", ", certainty + generateDateString(archDescDate.getIsoDateBegin(), archDescDate.getIsoDateEnd()));
			}

			//calculate min and max dates
			beginDate = ATDateUtils.getYearFromISODate(archDescDate.getIsoDateBegin());
			endDate = ATDateUtils.getYearFromISODate(archDescDate.getIsoDateEnd());
			if (beginDate < dateMin) {
				dateMin = beginDate;
			}
			if (endDate > dateMax) {
				dateMax = endDate;
			...
			if (archDescDate.getIsoBulkDateBegin() != null && archDescDate.getIsoBulkDateBegin().length() > 0) {
				if (dateBulk.length() == 0) {
					dateBulk = "bulk " + generateDateString(archDescDate.getIsoBulkDateBegin(), archDescDate.getIsoBulkDateEnd());
				} else {
					dateBulk = StringHelper.concat(", ", dateBulk, generateDateString(archDescDate.getIsoBulkDateBegin(), archDescDate.getIsoBulkDateEnd()));
				}
			}
	...
	private String generateDateString(String min, String max) {
...
//			createSubField(dataField, "f", resource.getDateExpression());
		...
		createSubField(dataField, "f", dateInclusive);
		createSubField(dataField, "g", dateBulk);
...
//		}
	...
	private void set300(RecordType record, Resources resource) {
		...
		for (ArchDescriptionPhysicalDescriptions physdesc: resource.getPhysicalDesctiptions()) {
			DataFieldType dataField = new DataFieldType();
			dataField.setTag("300");
			if (resource.getLevel().equalsIgnoreCase("item"))
				createSubField(dataField, "a", "1 item");
			else {
				if (physdesc.getExtentNumber() != null)
					try {
						createSubField(dataField, "a",
								physdesc.getExtentDisplay());
					} catch (ParseException e) {
						e.printStackTrace();
					}
			}
			createSubField(dataField, "a", physdesc.getContainerSummary());
			createSubField(dataField, "b", physdesc.getPhysicalDetail());
			createSubField(dataField, "c", physdesc.getDimensions());
	...
	private void createSubField(DataFieldType dataField, String subCode,
		...
		System.out.println("Create subfiled date field: " + dataField + " subCode: " + subCode + " subcode value: " + subFieldValue);
...
//			controlFieldString.append("i");
		...
		if (dateMax == 0) {
			...
			controlFieldString.append("s" + Integer.toString(dateMin) + "    ");
		} else {
			controlFieldString.append("i" + Integer.toString(dateMin) + Integer.toString(dateMax));
		}
...
//			controlFieldString.append("    ");
	...
	private void set650(RecordType record, Subjects subject) {
		...
		createSubField(dataField, "a", subject.getSubjectTerm());
		if (dataField.getInd2().equals("7"))
			createSubField(dataField, "2", LookupListUtils.getLookupListCodeFromItem(Subjects.class, Subjects.PROPERTYNAME_SUBJECT_SOURCE, subject.getSubjectSource()));
		if (dataField.getSubfield().size() > 0)
			record.getDatafield().add(dataField);
